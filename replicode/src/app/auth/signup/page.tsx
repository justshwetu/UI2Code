...
export default function SignupPage() {
  ...
  const submitSignup = async () => {
    ...
    setLoading(false);
    if (data.ok) { setOtpSent(true); setDevOtp(data.devOtp || ""); setPendingToken(data.pendingToken || ""); } else setMessage(data.error || "Error");
  };

  const verifyOtp = async () => {
    setLoading(true);
    setMessage("");
    const res = await fetch("/api/auth/verify-signup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, otp, pendingToken }) });
    const data = await res.json();
    setLoading(false);
    if (data.ok) { router.push("/dashboard"); } else setMessage(data.error || "Error");
  };
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/80 backdrop-blur p-8 shadow transition-transform duration-300 hover:-translate-y-0.5">
        ...
        {!otpSent ? (
          <div className="mt-6 space-y-4">
            ...
            <button onClick={submitSignup} disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-3 shadow-lg shadow-blue-900/30">{loading ? "Please wait" : "Sign up"}</button>
            <p className="text-sm text-slate-400">Already have an account? <Link href="/auth/login" className="text-blue-400">Log in</Link></p>
          </div>
        ) : (
          <div className="mt-6 space-y-4">
